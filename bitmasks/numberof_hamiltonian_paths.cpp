...
int main(){
    ...
    int dp[(1 << n)][n];
    // dp[mask][i] be the number of Hamiltonian walks in the subgraph generated by vertices in mask, that ends in the vertex i
    ...
    for(int mask = 0;mask < (1 << n); mask++){
    	for(int i = 0;i < n; i++){
    		...
    		if(bit(i, mask) && countsetbits(mask) == 1)
    			...
    			for(int j = 0;j < n; j++){
    				if(adj[j][i])
    					dp[mask][i] = dp[mask][i] + dp[mask^(1<<i)][j] + adj[j][i];
    			}
    		}
    	}
    }
